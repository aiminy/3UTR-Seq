...
convertBam2StrandBw <- function(input.bam.file.dir,input.chromosome.size.file,output.bw.file.dir){
  ...
  cmd.l <- lapply(1:length(res), function(u,m.id,res,output.bw.file.dir)
  {
    ...
    if(m.id == 1){
      ...
      u=1
      job.name=paste0("Bdg.",u)
      wait.job.name=paste(paste0("bamIndex.",u,"\")"),"&& done(\"Chrosome\")",sep=" ")
      cmd1 = paste0("bsub -w \"done(\"",wait.job.name,"\"",
                    " -P bbc -J \"",job.name,paste0("\" -o %J.",job.name,".log "),paste0("-e %J.",job.name,".err -W"))
  ...
  cmd.l <- lapply(1:length(res), function(u,m.id,res,output.bw.file.dir)
  {
    ...
    if(m.id == 1){
      ...
      wait.job.name=paste(paste0("Bdg.",u),"&& done(\"Chrosome\")",sep=" ")
      cmd1 = paste0("bsub -w \"done(\"",wait.job.name,"\"",
                    " -P bbc -J \"",job.name,paste0("\" -o %J.",job.name,".log "),paste0("-e %J.",job.name,".err -W"))
  ...
  cmd.l <- lapply(1:length(res), function(u,m.id,res,input.chromosome.size.file.m,output.bw.file.dir)
  {
    #path_name = dirname(u)
    #path_name2 <- basename(path_name)
    ...
    if(m.id == 1){
      ...
      wait.job.name=paste0("sortedBdg.",u)
      cmd1 = paste0("bsub -w \"done(\"",wait.job.name,"\")\"",
                    " -P bbc -J \"",job.name,paste0("\" -o %J.",job.name,".log "),paste0("-e %J.",job.name,".err -W"))
    ...
    cmd.y <- paste(cmd19,y,input.chromosome.size.file.m,file.path(output.bw.file.dir,paste0(file_name,"_minus.bw")), sep = " ")
